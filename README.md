...
Explore the [tutorial](https://github.com/band-unfolding/banduppy/tree/RestructuringCode/tutorials) folder for detailed examples. Below are quick snippets showcasing what you can achieve with `BandUPpy`:
...
```
    ...
    = band_unfold.Unfold(bands, kline_discontinuity_threshold = 0.1, 
                        ...
                        save_unfolded_bandstr = {'save2file': True, 
                                                ...
                                                'fname_suffix': ''})
```
#### 4. Plot unfolded band structure (fatband/density plot)
##### --------------------- Plot band structure ---------------------------------
```
    # Fermi energy
    Efermi = 5.9740
...
```
    fig, ax, CountFig \
    = band_unfold.plot_ebs(kpath_in_angs=kpline1, 
                            unfolded_bandstructure=unfolded_bandstructure_1, 
                            save_figure_dir=results_dir1, 
    ...
    _, ax, CountFig \
    = band_unfold.plot_ebs(ax=ax, kpath_in_angs=kpline2, 
                            unfolded_bandstructure=unfolded_bandstructure_2, 
                            save_figure_dir=results_dir2, threshold_weight=0.1,
                            save_file_name=save_file_name, CountFig=None, 
                            Ef=5.9786, Emin=Emin, Emax=Emax, pad_energy_scale=0.5, 
                            mode="fatband", special_kpoints=special_kpoints_pos_labels2, 
                            plotSC=False, marker='x', fatfactor=20, nE=100, smear=0.2,
                            scatter_color='black', color_map='viridis')
```
<!-- =========================================================== -->
